...
contract InsuranceSystem {
    ...
    function getAllInsurers() public view returns (User[] memory) {
        User[] memory allInsurers = new User[](insurerCount);
        for (uint256 i = 0; i < insurerCount; i++) {
            allInsurers[i] = insurers[insurerAddresses[i]];
        }
        return allInsurers;
    }

    function getAllHospitals() public view returns (User[] memory) {
        User[] memory allHospitals = new User[](hospitalCount);
    ...
    function getAllPolicyholders() public view returns (User[] memory) {
        User[] memory allPolicyholders = new User[](policyholderCount);
        for (uint256 i = 0; i < policyholderCount; i++) {
            allPolicyholders[i] = policyholders[policyholderAddresses[i]];
        }
